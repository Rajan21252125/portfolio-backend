...
router.get("/", async (req: Request, res: Response) => {
  ...
  try {
    const result = await query(
      "SELECT * FROM projects WHERE profile_id = 1 ORDER BY created_at DESC"
    );
    ...
    const duration = Date.now() - start;
    logger.error(
      { route: "GET /projects", error: err, durationMs: duration },
...
router.get("/:id", async (req: Request, res: Response) => {
  ...
  try {
    const result = await query(
      "SELECT * FROM projects WHERE id = $1 ORDER BY created_at DESC", [req.params.id]
    );
    ...
    const duration = Date.now() - start;
    logger.error(
      { route: "GET /projects/:id", error: err, durationMs: duration },
      "Failed to fetch projects"
    );
    return res.status(500).json({ message: "Server error" });
  }
...
router.post(
  ...
  uploadMemory.fields([{ name: "image", maxCount: 1 }, { name: "video", maxCount: 1 }]),
  ...
  async (req: Request, res: Response) => {
    ...
    if (!parsed.success) {
      ...
      logger.warn(
        { adminId: admin?.userId, fieldErrors: formatted.fieldErrors },
    ...
    let imageSecureUrl: string | null = null;
    let imagePublicId: string | null = null;
    let videoSecureUrl: string | null = null;
    ...
    try {
      ...
      if (imagePublicId) {
        try { await cloudinary.uploader.destroy(imagePublicId, { resource_type: "image" }); }
        catch (cleanupErr) { logger.warn({ cleanupErr, imagePublicId }, "Failed to cleanup image"); }
      }
      if (videoPublicId) {
        try { await cloudinary.uploader.destroy(videoPublicId, { resource_type: "video" }); }
        catch (cleanupErr) { logger.warn({ cleanupErr, videoPublicId }, "Failed to cleanup video"); }
      }
    ...
    try {
      ...
      if (imagePublicId) {
        try { await cloudinary.uploader.destroy(imagePublicId, { resource_type: "image" }); }
        catch (cleanupErr) { logger.warn({ cleanupErr, imagePublicId }, "Failed to cleanup image after DB error"); }
      }
      if (videoPublicId) {
        try { await cloudinary.uploader.destroy(videoPublicId, { resource_type: "video" }); }
        catch (cleanupErr) { logger.warn({ cleanupErr, videoPublicId }, "Failed to cleanup video after DB error"); }
      }
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (typeof raw.tools === "string") {
        try { raw.tools = JSON.parse(raw.tools); } catch { raw.tools = raw.tools.split(",").map((s: string) => s.trim()).filter(Boolean); }
      }
      ...
      let newVideoPublicId: string | null = null;
      ...
      try {
        ...
        if (newImagePublicId) { try { await cloudinary.uploader.destroy(newImagePublicId, { resource_type: "image" }); } catch (_) {} }
        if (newVideoPublicId) { try { await cloudinary.uploader.destroy(newVideoPublicId, { resource_type: "video" }); } catch (_) {} }
        return res.status(500).json({ message: "File upload failed" });
      }
      ...
      try {
        ...
        logger.info({
          ...
          entityId: updated.id,
          changes,
          meta: { route: "PUT /projects/:id", durationMs: Date.now() - start, ip: (req as any).ip },
        });
      } catch (auditErr) {
