...
import { addProfileDetails, updateProfileSql } from "../db/query.js";
...
router.post("/", requireAuth, uploadMemory.fields([
    ...
    try {
        ...
        if (profileFile) {
            ...
            try {
                ...
                const uploadRes = await uploadBuffer(profileFile.buffer, {
                    folder,
                    resource_type: "raw",
                });
        ...
        if (resumeFile) {
            ...
            try {
                ...
                const resCloud = await uploadBuffer(resumeFile.buffer, {
                    folder,
                    resource_type: "raw",
                });
                ...
                if (profilePicturePublicId) {
                    try {
                        await cloudinary.uploader.destroy(profilePicturePublicId, { resource_type: "image" });
                    }
                    catch (_) { }
                }
        ...
        logger.error({ adminId: req.user?.userId, err }, "Failed to create profile (server error)");
...
uploadMemory.fields([
    ...
    try {
        ...
        let newProfilePicturePublicId = null;
        let newResumeUrl = null;
        let newResumePublicId = null;
        ...
        if (profileFile) {
            ...
            try {
                const folder = `${process.env.CLOUDINARY_FOLDER ?? "portfolio"}/profiles`;
                const rc = await uploadBuffer(profileFile.buffer, { folder, resource_type: "image", transformation: [{ width: 800, crop: "limit" }] });
        ...
        if (resumeFile) {
            ...
            try {
                const folder = `${process.env.CLOUDINARY_FOLDER ?? "portfolio"}/profiles/resumes`;
                const rr = await uploadBuffer(resumeFile.buffer, { folder, resource_type: "raw" });
                ...
                if (newProfilePicturePublicId) {
                    try {
                        await cloudinary.uploader.destroy(newProfilePicturePublicId, { resource_type: "image" });
                    }
                    catch (_) { }
                }
