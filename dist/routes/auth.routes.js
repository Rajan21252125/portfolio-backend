...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        await query(insertOTP, [email, otpHash, expiresAt]);
        logger.info({ email }, "OTP generated and stored in DB");
        try {
            await sendTemplatedEmail("mailTemplates/user_password_email.html", { appName: process.env.APP_NAME || "server.portfolio", userName: email, otpOrCode: otp, codeTtl: OTP_TTL_MINUTES, supportEmail: process.env.SUPPORT_EMAIL }, { to: email, subject: "Your login verification code" });
...
router.post("/verify-otp", async (req, res) => {
    ...
    try {
        ...
        const otpRes = await query(getOtpByEmail, [email]);
        ...
        if (!isValid) {
            ...
            await query(`UPDATE login_otps SET attempts = COALESCE(attempts, 0) + 1 WHERE id = $1`, [record.id]);
        ...
        // create token
        const token = jwt.sign({ userId: user.id, email: user.email, isAdmin: user.is_admin }, process.env.JWT_SECRET, { expiresIn: "1d" });
...
router.post("/sign-up", async (req, res) => {
    ...
    try {
        ...
        const expiresAt = new Date(Date.now() + OTP_TTL_MINUTES * 60 * 1000);
        const result = await query(signUpQuery, [email, hashed, tokenHash, expiresAt]);
        const newUser = result.rows[0];
        logger.info({ email, userId: newUser.id }, "User created successfully");
        // create admin notification row
        await query(adminNotificationInsert, [newUser.id, JSON.stringify({ email })]);
        // send verification mail to user
        const verifyUrl = `${process.env.APP_URL}/auth/verify-email?token=${token}&email=${encodeURIComponent(email)}`;
        try {
            await sendTemplatedEmail("mailTemplates/user_verification_email.html", { appName: process.env.APP_NAME || "server.portfolio", userName: email, verifyUrl, tokenTtl: OTP_TTL_MINUTES, supportEmail: process.env.SUPPORT_EMAIL }, { to: email, subject: "Verify your email address" });
        ...
        try {
            await sendTemplatedEmail("mailTemplates/admin_notify_new_user.html", { appName: process.env.APP_NAME || "server.portfolio", userEmail: email, createdAt: new Date().toISOString(), supportEmail: process.env.SUPPORT_EMAIL }, { to: ADMIN_EMAIL, subject: `New signup: ${email}` });
        }
        catch (adminEmailErr) {
...
router.get("/verify-email", async (req, res) => {
    ...
    try {
        ...
        const row = await query(`SELECT id, verification_token_expires_at FROM users WHERE email = $1 AND verification_token = $2 LIMIT 1`, [String(email), tokenHash]);
...
router.get("/admin/pending-users", requireAuth, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const usersRes = await query(`SELECT id, email, created_at, is_verified
       FROM users
       WHERE is_approved = FALSE
       ORDER BY created_at ASC`);
        const approvedRes = await query(`SELECT id, email, created_at
      FROM users
      ...
      ORDER BY created_at DESC`);
        ...
        const notesRes = await query(`SELECT id, user_id, type, payload, read, created_at
 ...
 */
router.post("/admin/approve-user/:id", requireAuth, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            await sendTemplatedEmail("../mailTemplates/user_approved_email.html", {
...
router.post("/resend-verification/:id", async (req, res) => {
    try {
        ...
        const userRes = await query("SELECT id, email, is_verified FROM users WHERE id = $1", [userId]);
        const user = userRes.rows[0];
        if (!user) {
            return res.status(404).json({ message: "User not found" });
        }
        ...
        await query(`INSERT INTO email_verification (email, token, expires_at)
       VALUES ($1, $2, $3)`, [user.email, token, expiresAt]);
        // generate verify URL
        ...
        try {
            await sendTemplatedEmail("../mailTemplates/user_verification_email.html", {
